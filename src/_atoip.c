...
inet_ipton4(const char *src, unsigned char *dst)
{
	...
	unsigned char tmp[_INADDRSZ], *tp;
...
inet_ipton6(const char *src, unsigned char *dst)
{
	...
	unsigned char tmp[_IN6ADDRSZ], *tp = 0, *endp = 0, *colonp = 0;
	const char *xdigits = 0, *curtok = 0;
	int ch = 0, saw_xdigit = 0, count_xdigit = 0;
	unsigned int val = 0;
	unsigned dbloct_count = 0;

	memset((tp = tmp), '\0', _IN6ADDRSZ);
	endp = tp + _IN6ADDRSZ;
	colonp = NULL;
...
_atoip(const char *buf, int flags, void *res, unsigned ressize)
{
	...
	if (res && ressize < sizeof(struct _ipaddr))
		return -1;
		...
		if (prefix == NULL) {
			...
			*prefix = '\0';
			...
			if (errno || (*prefix_end != '\0')
			    || prefixlen < 0 || prefixlen > _PREFIXMAX)
				return -1;
